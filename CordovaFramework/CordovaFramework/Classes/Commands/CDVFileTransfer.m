...
- (NSURLRequest*)requestForUploadCommand:(CDVInvokedUrlCommand*)command fileData:(NSData*)fileData
{
    ...
    if (chunkedMode) {
        ...
        /*
        self.backgroundTaskID = [[NSApplication sharedApplication] beginBackgroundTaskWithExpirationHandler:^{
                [[NSApplication sharedApplication] endBackgroundTask:self.backgroundTaskID];
...
- (void)connectionDidFinishLoading:(NSURLConnection*)connection
{
    ...
    /*
    [[NSApplication sharedApplication] endBackgroundTask:self.command.backgroundTaskID];
